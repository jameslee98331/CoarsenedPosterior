...
for (i_a, alpha) in enumerate(alphas)
    # Sample data
    x = [(rand() < p1 ? skewrnd(1, l1, s1, a1) : skewrnd(1, l2, s2, a2))[1]::Float64 for j = 1:n]
    ...
    ylim(0, 0.4)
    if alpha < Inf
        title("\$\\mathrm{Coarsened\\,posterior}\$", fontsize=17)
    else
        title("\$\\mathrm{Standard\\,posterior}\$", fontsize=17)
    end
